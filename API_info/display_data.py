...
# Display the upcoming MLB games and point spreads
...
def generate_expander(data_obj, button_id):
    ...
    with col2:
        expander_string = f"{away_team} @ {home_team} → → → → O/U: {over_under}"
        with st.expander(expander_string, expanded=False):
            # create columns within the expander to display team logs
            col1, col2, col3 = st.columns([1, 3, 1])
            with col1:
                st.image(f"images_mlb/{away_team.lower()}.png", width=100)
                
            with col2:  
                st.title(f"@", text_alignment='center') 
            with col3:
                st.image(f"images_mlb/{home_team.lower()}.png", width=100)
...
def handle_change(changed_key, game_info):
    ...
    if not st.session_state.point_picks:
        add_new_pick_to_session_state(
            ...
            value_of_pick if key_type == "OU" else None,
            over_under_score,
            game_id,
            button_id
        )
        ...
        for existing_pick in st.session_state.point_picks:
            ...
            if existing_game_id == game_id:
                ...
                if key_type == "points":
                    ...
                    existing_pick['over_under'] = value_of_pick
            ...
            if key_type == "points":
                if value_of_pick == existing_pick['point_value'] and not same_id:
                    ...
                    recrafted_key2 = str(existing_pick['button_id']) + "_OU"
