...
def create_user_db_entry(data_entry):
    conn = st.connection("dev_picks", type=SupabaseConnection)
    conn.table("dev_picks").insert(data_entry).execute()
...
def get_all_user_picks():
    conn = st.connection("dev_picks", type=SupabaseConnection)
    rows = conn.table("dev_picks").select("*").execute().data
    return rows

# Get the picks for one single user
def get_user_picks(name):
    conn = st.connection("dev_picks", type=SupabaseConnection)
    row = conn.table("dev_picks").select("*").eq("name", name).execute().data
    return row
...
def get_picks_by_week(week_number, name):
    ...
    conn = st.connection("dev_picks", type=SupabaseConnection)
    if name is None:
        rows = conn.table("dev_picks").select(week).execute().data
    else:
        rows = conn.table("dev_picks").select(week).eq("name", name).execute().data
    return rows
...
def get_point_total_for_week(week_number, name):
    ...
    conn = st.connection("dev_picks", type=SupabaseConnection)
    row = conn.table("dev_picks").select(point_total_col_name).eq("name", name).execute().data
...
def get_win_loss_by_week(week_number, name):
    conn = st.connection("dev_picks", type=SupabaseConnection)
    row = conn.table("dev_picks").select(f"win_loss_week_{week_number}").eq("name", name).execute().data
...
def get_win_loss_record(name):
    conn = st.connection("dev_picks", type=SupabaseConnection)
    row = conn.table("dev_picks").select("win_loss_record").eq("name", name).execute().data
...
def get_user_points():
    ...
    for name in name_list:
        row = conn.table("dev_picks").select("accumulated_points").eq("name", name).execute().data
        if row:
...
def get_week_number():
    conn = st.connection("dev_picks", type=SupabaseConnection)
    row = conn.table("dev_picks").select("current_week").eq("name", "Tyler").execute().data
...
def update_user_picks(name, data_entry):
    conn = st.connection("dev_picks", type=SupabaseConnection)
    conn.table("dev_picks").update(data_entry).eq("name", name).execute()
...
def update_win_loss_info(name, correct_picks, incorrect_picks, push_picks):
    ...
    conn.table("dev_picks").update({"win_loss_record": win_loss_json}).eq("name", name).execute()
