...
def display_player_picks(home_team, away_team, point_spread, point_value, spread_pick, score_home_team, score_away_team, covering_spread):
    ...
    with col2:
        if spread_pick == "EVEN":
            ...
            expander_string = f"{point_value} POINT PLAY: {away_team} @ {home_team} → → → → YOUR PICK: {spread_pick} → → → → {'✅' if covering_spread else '❌'}"
        with st.expander(expander_string, expanded=False):
            ...
            if spread_pick == "EVEN":
                ...
                st.markdown(f"## **SPREAD**: {point_spread}", text_alignment='center')
